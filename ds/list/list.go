...
func New() *List {
	return &List{record: make(Record), values: make(map[string]map[string]int)}
}
...
// RPush return number of value in key
...
func (lis *List) LRem(key string, val []byte, count int) int {
	...
	if count > 0 {
		for p := item.Front(); len(ele) < count && p != nil; p = p.Next() {
			if reflect.DeepEqual(p.Value.([]byte), val) {
				ele = append(ele, p)
			}
		}
	...
	if count < 0 {
		for p := item.Back(); len(ele) < -count && p != nil; p = p.Prev() {
			if reflect.DeepEqual(p.Value.([]byte), val) {
				ele = append(ele, p)
			}
		}
...
func (lis *List) LRange(key string, start, end int) [][]byte {
	...
	if end <= mid || end-mid < mid-start {
		flag := 0
		for p := item.Front(); p != nil && flag <= end; p = p.Next() {
			if flag >= start {
		...
		flag := length - 1
		for p := item.Back(); p != nil && flag >= start; p = p.Prev() {
			if flag <= end {
				val = append(val, p.Value.([]byte))
			}
			flag--
...
func (lis *List) validIndex(key string, index int) (bool, int) {
	if item := lis.record[key]; item != nil && item.Len() > 0 {
		length := item.Len()
		if index < 0 {
			index += length
		}
