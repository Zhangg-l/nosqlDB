...
// 	t.Log(ok)
// 	ok, _ = db.HSetNx([]byte(key), []byte("my_new_lang"), []byte("Jaava Go Python"))
...
// // 	defer db.Close()
// // 	var res []byte
// // 	 res = db.HGet([]byte(key), []byte("my_name"))
// // 	fmt.Println(res)
// // 	res = db.HGet([]byte(key), []byte("not exist"))
// // 	fmt.Println(res)
// // 	res = db.HGet([]byte(key), []byte("my_hobby"))
// // 	fmt.Println(res)
...
// // 	res, _ := db.HDel([]byte(key), []byte("my_hobby"), []byte("my_name2"), []byte("my_name3"))
// // 	t.Log(res)
// // 	t.Log(db.HLen([]byte(key)))
// // }
...
// // 	err = db.HExpire(key, 7)
// // 	fmt.Println(db.HGetAll(key))
// // 	assert.Equal(t, err, nil)
...
// // 		assert.NotNil(t, err)
// // 		assert.EqualError(t, err, "rosedb: wrong number of arguments")
// // 	})
...
// // 		assert.EqualError(t, err, "rosedb: value exceeded the max length")
